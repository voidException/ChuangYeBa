...
// Su Ziming Added For this program
@property (nonatomic, strong) UIView *tabViewContent;
@end
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.selected) {
        ...
        // Draw the indicator
        // Su Ziming Modified For this program
        float offsetMargin = 0.0;
        [bezierPath moveToPoint:CGPointMake(offsetMargin, CGRectGetHeight(rect) - 1.0)];
        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect) - offsetMargin, CGRectGetHeight(rect) - 1.0)];
        [bezierPath setLineWidth:1.0];
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    /*
     * Su Ziming replace this method to
     */
    /*
    ...
    [activeTabView.tabViewContent removeFromSuperview];
    
    UIView *tabViewContent = [self.dataSource viewPager:self viewForTabAtIndex:self.activeTabIndex];
    ...
    activeTabView.tabViewContent = tabViewContent;
    float centerY = [self.tabHeight floatValue]/2;
    float centerX = [self.tabWidth floatValue]/2;
    tabViewContent.center = CGPointMake(centerX, centerY);
    [activeTabView addSubview:activeTabView.tabViewContent];
    ...
    [activeTabView.tabViewContent removeFromSuperview];
    tabViewContent = [self.dataSource viewPager:self activeViewForTabAtIndex:activeTabIndex];
    tabViewContent.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    // Create TabView and subview the content
    activeTabView.tabViewContent = tabViewContent;
    centerY = [self.tabHeight floatValue]/2;
    centerX = [self.tabWidth floatValue]/2;
    tabViewContent.center = CGPointMake(centerX, centerY);
    [activeTabView addSubview:activeTabView.tabViewContent];
    /*
     * Replace End!
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        /**
         * Su Ziming Modified replace this method for this program
         *
